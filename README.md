...
未完成且设置了截止时间的待办会自动生成日程，通过 ICS 订阅同步到 Windows 日历：

1. 打开侧边栏 ✅ 待办面板，复制底部的订阅地址（形如 `http://127.0.0.1:3080/dsh-sidebar-todos.ics`）。
2. 在 Windows 日历中订阅该地址：
   - **Windows 11 日历（Outlook 新体验）**：设置 → 日历 → 添加日历 → 从 Web 订阅 → 粘贴地址。
   - **Windows 10 邮件和日历**：设置 → 管理日历 → 添加日历 → 从互联网订阅 → 粘贴地址。
3. 日历 App 会定期刷新该订阅；每次待办增删改后插件立即更新 ICS。

规则：仅**未完成且有截止时间**的待办进入日历；标题带优先级前缀（`[高]/[中]/[低]`）；仅日期按全天日程，日期+时间按 1 小时日程；完成后自动从日历移除（下次刷新）。

> 注意：订阅依赖本机 DSH 运行中（`http://127.0.0.1:3080`）；若日历 App 不支持订阅，也可把该 URL 打开后另存为 `.ics` 文件手动导入（一次性）。
...
- **v9** (`pkg-10`)：Windows 日历同步 —— Host 生成 ICS，经 `webServer` 提供订阅 URL（`/dsh-sidebar-todos.ics`），未完成且有截止时间的待办自动成为日程，变更即时刷新。
