  ...
  apply(ctx) {
    ...
    function TodoView(props) {
      ...
      const add = () => {
        ...
        if (!t) return
        setText('')
        host.call('todo.add', { text: t }).then((res) => {
    ...
    function ToolsSidebar() {
      ...
      const toggleDiag = () => {
        ...
        try {
          ...
          const root = q('.dsh-sidebar-root')
          const rail = q('.dsh-sidebar-rail')
          const btn = q('.dsh-sidebar-btn')
          const cs = (el) => (el ? getComputedStyle(el) : null)
          const rootStyle = cs(root)
          const railStyle = cs(rail)
          const btnStyle = cs(btn)
          setDiag({
            '深色模式': document.body && document.body.hasAttribute('data-ds-dark-theme') ? '是' : '否',
            'color-scheme': document.documentElement ? String(document.documentElement.style.colorScheme || '') : '',
            'rail 背景': railStyle ? railStyle.backgroundColor : '',
            '按钮文字色': btnStyle ? btnStyle.color : '',
            '按钮字号': btnStyle ? btnStyle.fontSize : '',
            'token label-secondary': rootStyle ? rootStyle.getPropertyValue('--dsw-alias-label-secondary').trim() : '',
            'token bg-layer-1': rootStyle ? rootStyle.getPropertyValue('--dsw-alias-bg-layer-1').trim() : '',
          })
        } catch (err) {
          setDiag({ '诊断出错': String((err && err.message) || err) })
        }
      }
      ...
      const rail = React.createElement('div', { className: 'dsh-sidebar-rail' },
        ...
        React.createElement('button', {
          className: 'dsh-sidebar-diag-btn',
          onClick: toggleDiag,
          title: '样式诊断',
        }, 'ⓘ'),
      )
      ...
      let diagPanel = null
      if (diag) {
        diagPanel = React.createElement('div', { className: 'dsh-diag' },
          React.createElement('div', { className: 'dsh-diag-row' },
            React.createElement('span', null, '样式诊断'),
            React.createElement('button', { className: 'dsh-icon-btn', onClick: () => setDiag(null), title: '关闭' }, '✕'),
          ),
          Object.keys(diag).map((k) =>
            React.createElement('div', { key: k, className: 'dsh-diag-row' },
              React.createElement('span', null, k),
              React.createElement('code', null, String(diag[k])),
            ),
          ),
        )
      }
