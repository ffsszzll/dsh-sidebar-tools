  ...
  apply(ctx) {
    ...
    function TodoView(props) {
      ...
      React.useEffect(() => {
        ...
        host.call('todo.syncInfo').then((res) => {
          if (!cancelled && res && res.url) setSyncUrl(res.url)
        }).catch(() => { /* ignore */ })
        return () => { cancelled = true }
      }, [])
      ...
      return React.createElement('div', { className: 'dsh-sidebar-panel' },
        ...
        listBody,
        syncUrl
          ? React.createElement('div', { className: 'dsh-todo-sync' },
              React.createElement('span', null, '日历订阅:'),
              React.createElement('a', {
                href: syncUrl,
                target: '_blank',
                rel: 'noreferrer',
                className: 'dsh-todo-sync-link',
              }, syncUrl),
            )
          : null,
      )
